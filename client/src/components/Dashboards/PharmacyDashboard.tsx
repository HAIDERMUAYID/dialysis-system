...
const PharmacyDashboard: React.FC = () => {
  ...
  const handleComplete = async (visitId: number) => {
    try {
      ...
      showMessage('success', 'تم صرف العلاج بنجاح');
      fetchVisits();
      if (selectedVisit === visitId) {
        setSelectedVisit(null);
      }
    } catch (error: any) {
      showMessage('error', error.response?.data?.error || 'حدث خطأ أثناء صرف العلاج');
    }
  };
  ...
  return (
    <div className="App">
      <nav className="navbar">
        <div className="navbar-content">
          <div>
            ...
            <div style={{ fontSize: '0.9rem', marginTop: '0.25rem' }}>الصيدلي</div>
          </div>
      ...
      <div className="container">
        ...
        <div className="card">
          <h2 className="card-header">الزيارات المعلقة - الصيدلية</h2>
